...
function getLevaSchemaFromShaders(
  vertexShader: string,
...
export default function guiShaderMaterial(
  passedUniforms = {},
  vertexShader,
  fragmentShader
) {
  ...
  const uniforms = getUniformsFromShader(
    passedUniforms,
    vertexShader,
    fragmentShader
  );
...
export function useMaterialGUI(name) {
  ...
  const onRef = useCallback(node => {
    if (node) {
      ...
      setControls(
        () => getLevaSchemaFromShaders(vertexShader, fragmentShader) ?? {}
      );
    }
  }, []);
