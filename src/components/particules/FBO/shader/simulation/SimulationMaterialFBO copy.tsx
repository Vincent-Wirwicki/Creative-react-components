...
const getRandomData = (width: number, height: number) => {
  ...
  for (let i = 0; i < length; i++) {
    ...
    data[stride] = distance * Math.cos(theta) * Math.cos(phi);
...
export default class SimulationMaterialFBO extends ShaderMaterial {
  constructor(size: number) {
    ...
    super({
      ...
      void main() {
        ...
        vec3 targetPos = pos + curlNoise(pos * uFrequency) * 96.;
        float dist = length(pos-targetPos) / 48.; 
        ...
        // pos = mix(pos, targetPos, pow(dist, 5.));
