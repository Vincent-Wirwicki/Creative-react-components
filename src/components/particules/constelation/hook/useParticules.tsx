...
const useParticules = (canvasRef: RefObject<HTMLCanvasElement | null>) => {
  //particules array
    ...
    if (canvasRef.current) {
      for (let i = 0; i < config.density; i++) {
        particulesArrayRef.current.push({
          radius: radius,
          pos: {
            x: randomizePos(radius, canvasRef.current.width),
            y: randomizePos(radius, canvasRef.current.height),
          },
          speed: {
            x: randomizeSpeed(),
            y: randomizeSpeed(),
  ...
  }, [canvasRef, config.density, radius]);
